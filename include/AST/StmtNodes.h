//
// Created by tangny on 2021/12/10.
//

#ifndef STMT
#define STMT(CLASS, BASE)
#endif

#ifndef COMPOUNDSTMT
#define COMPOUNDSTMT(CLASS, BASE) STMT(CLASS, BASE)
#endif
COMPOUNDSTMT(CompoundStmt, Stmt)
#undef COMPOUNDSTMT

#ifndef DECLSTMT
#define DECLSTMT(CLASS, BASE) STMT(CLASS, BASE)
#endif
DECLSTMT(DeclStmt, Stmt)
#undef DECLSTMT

#ifndef WHILESTMT
#define WHILE(CLASS, BASE) STMT(CLASS, BASE)
#endif
WHILE(WhileStmt, Stmt)
#undef WHILE

#ifndef DOWHILESTMT
#define DOWHILE(CLASS, BASE) STMT(CLASS, BASE)
#endif
DOWHILE(DoWhileStmt, Stmt)
#undef DOWHILE

#ifndef FORSTMT
#define FORSTMT(CLASS, BASE) STMT(CLASS, BASE)
#endif
FORSTMT(ForStmt, Stmt)
#undef FORSTMT

#ifndef IFSTMT
#define IFSTMT(CLASS, BASE) STMT(CLASS, BASE)
#endif
IFSTMT(IfStmt, Stmt)
#undef IFSTMT

#ifndef VALUESTMT
#define VALUESTMT(CLASS, BASE) STMT(CLASS, BASE)
#endif
VALUESTMT(ValueStmt, Stmt)
#undef VALUESTMT

#ifndef RETURNSTMT
#define RETURNSTMT(CLASS, BASE) STMT(CLASS, BASE)
#endif
RETURNSTMT(ReturnStmt, Stmt)
#undef RETURNSTMT

#ifndef EXPR
#define EXPR(CLASS, BASE) STMT(CLASS, BASE)
#endif
EXPR(Expr, ValueStmt)
#undef EXPR

#ifndef UNARYOPEROTR
#define UNARYOPEROTR(CLASS, BASE) STMT(CLASS, BASE)
#endif
UNARYOPEROTR(UnaryOperator, Expr)
#undef UNARYOPEROTR

#ifndef SELECTOR
#define SELECTOR(CLASS, BASE) STMT(CLASS, BASE)
#endif
SELECTOR(Selector, Expr)
#undef SELECTOR

#ifndef DEREFSELECTOR
#define DEREFSELECTOR(CLASS, BASE) STMT(CLASS, BASE)
#endif
DEREFSELECTOR(DerefSelector, Selector)
#undef DEREFSELECTOR

#ifndef INDEXSELECTOR
#define INDEXSELECTOR(CLASS, BASE) STMT(CLASS, BASE)
#endif
INDEXSELECTOR(IndexSelector, Selector)
#undef INDEXSELECTOR

#ifndef FIELDSELECTOR
#define FIELDSELECTOR(CLASS, BASE) STMT(CLASS, BASE)
#endif
FIELDSELECTOR(FieldSelector, Selector)
#undef FIELDSELECTOR

#ifndef SELECTORARRAY
#define SELECTORARRAY(CLASS, BASE) STMT(CLASS, BASE)
#endif
SELECTORARRAY(SelectorArray, Expr)
#undef SELECTORARRAY

#ifndef BINARYOPEROTR
#define BINARYOPEROTR(CLASS, BASE) STMT(CLASS, BASE)
#endif
BINARYOPEROTR(BinaryOperator, Expr)
#undef BINARYOPEROTR

#ifndef CASTEXPR
#define CASTEXPR(CLASS, BASE) STMT(CLASS, BASE)
#endif
CASTEXPR(CastExpr, Expr)
#undef CASTEXPR

#ifndef CALLEXPR
#define CALLEXPR(CLASS, BASE) STMT(CLASS, BASE)
#endif
CALLEXPR(CallExpr, Expr)
#undef CALLEXPR

#ifndef DECLREFEXPR
#define DECLREFEXPR(CLASS, BASE) STMT(CLASS, BASE)
#endif
DECLREFEXPR(DeclRefExpr, Expr)
#undef DECLREFEXPR

#ifndef INTERGERLITERAL
#define INTERGERLITERAL(CLASS, BASE) STMT(CLASS, BASE)
#endif
INTERGERLITERAL(IntegerLiteral, Expr)
#undef INTERGERLITERAL

#ifndef FLOATINGLITERAL
#define FLOATINGLITERAL(CLASS, BASE) STMT(CLASS, BASE)
#endif
FLOATINGLITERAL(FloatingLiteral, Expr)
#undef FLOATINGLITERAL

#ifndef STRINGLITERAL
#define STRINGLITERAL(CLASS, BASE) STMT(CLASS, BASE)
#endif
STRINGLITERAL(StringLiteral, Expr)
#undef ISTRINGLITERAL

#ifndef EXPLICITCASTEXPR
#define EXPLICITCASTEXPR(CLASS, BASE) STMT(CLASS, BASE)
#endif
EXPLICITCASTEXPR(ExplicitCastExpr, CastExpr)
#undef EXPLICITCASTEXPR

#ifndef IMPLICITCASTEXPR
#define IMPLICITCASTEXPR(CLASS, BASE) STMT(CLASS, BASE)
#endif
IMPLICITCASTEXPR(ImplicitCastExpr, CastExpr)
#undef IMPLICITCASTEXPR

#undef STMT