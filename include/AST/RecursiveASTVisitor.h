...
#define DECL(CLASS, BASE)                                                       \
   bool walkUpFrom##CLASS(CLASS *D) {                                           \
     ...
     if(getDerived().walkUpToTop())                                             \
       res = walkUpFrom##BASE(D);                                               \
     return (getDerived().visit##CLASS(D) & res);                               \
   }                                                                            \
...
#define STMT(CLASS, BASE)                                                       \
   bool walkUpFrom##CLASS(CLASS *S) {                                           \
     bool res = true;                                                           \
     if(getDerived().walkUpToTop())                                             \
       res = walkUpFrom##BASE(S);                                               \
     return (getDerived().visit##CLASS(S) & res);                               \
   }                                                                            \
...
void RecursiveASTVisitor<Derived>::traverseExprHelper( Expr *expr, std::string ctx ) {
    switch(expr->getKind()) {
        ...
        case(Stmt::k_CallExpr):
            traverseCallExpr(dynamic_cast<CallExpr*>(expr));
            break;
        default:
...
void RecursiveASTVisitor<Derived>::traverseStmtHelper( Stmt *stmt, std::string ctx ) {
    switch(stmt->getKind()) {
        ...
        case(Stmt::k_IfStmt):
            ...
            break;
        default:
            traverseExprHelper(dynamic_cast<Expr*>(stmt), ctx);
    }
}
